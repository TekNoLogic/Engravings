...
def export(filename, title, data, restrict_class)
...
def mine_item_weights(wh)
	...
	wh.get_weight_filters.each_pair do |c,specs|
		...
		specs.each_pair do |spec,w|
			...
			export(File.join("Data", "Wowhead#{c.to_s.capitalize}#{spec.gsub(/ /, "_").capitalize}.lua"), spec, data.compact.sort.map {|d| d.join(" ")}.join("\n"), c.to_s.upcase.gsub("_", ""))
		end
	end
end
